...
*/
template<typename T>
void LinkedList<T>::insert(T value, int pos) { // y posicion
    Node<T> *newNode = new Node<T>(value);
    if(head==nullptr){
        head = newNode;
    } else {
...
void LinkedList<T>::end(){
    ...
    Node<T> *tmp = head;
    if(head){
        while(tmp->getNext() != nullptr){  //hasta que llegue al final de la lista
            tmp= tmp->getNext();
        }
        return tmp; // el acutal es al ultimo nodo añadido
    }else{
        head = newNode; // si esta vacia, dirige el apuntador head al nuevo nodo
    }
}
